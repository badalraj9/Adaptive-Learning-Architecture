...
class DynamicTopology(nn.Module):
    ...
    def forward(self, Phi):
        """
        Phi: unified field ∈ R^(batch, d)
        H_t = (A ⊙ W_weight) · Phi  — A masks the weight matrix, not the output
        returns H_t ∈ R^(batch, d)
        """
        W_masked = self.A * self.W.weight   # (d, d) element-wise mask
        H        = Phi @ W_masked.T         # (batch, d)
        return F.gelu(H)
